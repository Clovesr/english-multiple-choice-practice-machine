...
| ID | 功能 | 优先级 | 状态 | 说明（Claude 初评） | 后端核对（Codex） |
|---|---|---|---|---|---|
| LIB-01 | 资源收件箱 | P0 / 核心版 | 🟡 | resources 状态机含 inbox/needs_review；独立收件箱工作流 UI 无 | 🟡 resources 有 inbox/needs_review，解析失败保留原文件；无独立整理事务 |
| LIB-02 | 文件导入 | P0 / 核心版 | 🟡 | PDF/DOCX/TXT/MD 导入、失败保留原文件及前端入口已完成；仍缺 EPUB、拖入/解析预览、大文件取消与续传 | 🟡 resources.py 支持 PDF/DOCX/TXT/MD、哈希与原件保留；tests/test_resources.py 覆盖 |
| LIB-03 | 媒体导入 | P1 / 稳定版 | ⬜ |  | ⬜ 无 media_assets 或媒体技术信息探测 |
...
| ID | 功能 | 优先级 | 状态 | 说明（Claude 初评） | 后端核对（Codex） |
|---|---|---|---|---|---|
| ACT-01 | 统一活动定义 | P0 / 核心版 | ⬜ |  | ⬜ 仅有 ACT_MODEL_DRAFT 研究候选；无 activities/activity_versions 实现 |
| ACT-02 | 活动渲染器注册 | P0 / 核心版 | ⬜ |  | ⬜ 无 renderer capability 合同或注册 API |
| ACT-03 | 单选 | P0 / 核心版 | 🟡 | 单选为旧机成熟能力；未入统一活动模型（D-02 逐步映射） | 🟡 旧 practice 使用 stable_key、保存 display order 并后端判分；未统一版本化 |
...
| ACT-15 | 作文 | P1 / 稳定版 | ⬜ |  | ⬜ 无作文版本、量表与正文绑定 |
...
| VOC-16 | 听音拼写卡 | P0 / 核心版 | ✅ | 听音拼写卡+逐字符判定 | ✅ listening/spelling 卡与统一 answer.accept 已实现 |
| VOC-17 | 例句挖空卡 | P1 / 稳定版 | ✅ | 例句挖空卡（语境来源） | ✅ cloze 卡按语境生成，缺语境不生成空卡 |
| VOC-18 | 搭配/词形卡 | P1 / 稳定版 | 🟡 | 搭配卡实现、无离线搭配数据默认关闭 | 🟡 collocation 卡引擎有；词形专项与默认数据不足 |
| VOC-19 | FSRS | P0 / 核心版 | ✅ | FSRS 词级调度（py-fsrs 6.x，0006 词级重构） | ✅ py-fsrs 固定版本、词级 0006、前后状态日志和幂等评分 |
| VOC-20 | 目标保持率 | P1 / 稳定版 | ⬜ |  | ⬜ 无目标保持率设置、模拟和参数重排 |
| VOC-21 | 新卡/复习上限 | P0 / 核心版 | ✅ | 每日新卡/复习上限+设置界面 | ✅ daily_new/daily_review_max 与会话配额后端已实现 |
| VOC-22 | 遗忘重学 | P0 / 核心版 | ✅ | 遗忘重学 learning steps+同日首评规则+当日重练内循环 | ✅ learning/relearning steps、同日首评和当日重练已实现 |
| VOC-23 | 顽固词处理 | P1 / 稳定版 | 🟡 | leech 阈值+统计显示；处置动作（拆卡/换材料）无 | 🟡 leech_threshold、lapses 和计数已有；无处置动作/备注/效果跟踪 |
| VOC-24 | 临时过滤复习 | P1 / 稳定版 | ⬜ |  | ⬜ 无 saved_query/filtered_deck 与是否重排选择 |
| VOC-25 | 积压与轻松日 | P1 / 稳定版 | 🟡 | 积压分批计划（spread/suspend_new/focus_overdue）；轻松日无 | 🟡 spread/suspend_new/focus_overdue 方案有；无 easy_days 和清空日期 |
| VOC-26 | 词汇统计 | P1 / 稳定版 | 🟡 | overview：保持率/到期预测/顽固数；按集合筛选无 | 🟡 overview 有保持率、连续天数和 7 日预测；集合/来源/词义筛选无 |
| VOC-27 | 批量导入/清洗 | P1 / 稳定版 | 🟡 | TXT/CSV 上传、词典匹配、基础去重、有限预热及 matched/unmatched 报告已完成 | 🟡 TXT/CSV、词典匹配、基础去重、有限预热有测试；字段映射/草稿/失败行修复无 |
| VOC-28 | 开放导出 | P1 / 稳定版 | ⬜ |  | ⬜ 无词汇 CSV/JSON/Markdown/Anki 导出 |
...
| REV-12 | 积压处理 | P1 / 稳定版 | ✅ | 积压分批（3/7/14 天计划） | ✅ backlog 3/7/14 日分批方案与应用接口已实现 |
...
| REV-20 | 复习导出与算法审计 | P2 / 高完成度 | ⬜ |  | ⬜ 无复习导出、参数/版本审计包 |
...
| ID | 功能 | 优先级 | 状态 | 说明（Claude 初评） | 后端核对（Codex） |
|---|---|---|---|---|---|
| DAT-01 | 数据目录分层 | P0 / 核心版 | 🟡 | db/resources/backups 分层；cache/logs 未独立 | 🟡 db/resources/backups 已分层；cache/logs/temp 根目录约束不完整 |
| DAT-02 | 数据库版本化迁移 | P0 / 核心版 | ✅ | 版本化迁移+校验和+漂移拒启（0001-0006） | ✅ schema_migrations、校验和、漂移拒启和 0001-0006 已测试 |
| DAT-03 | 迁移前自动备份 | P0 / 核心版 | ✅ | 迁移前自动备份 | ✅ runner 应用迁移前创建一致备份，失败保留 |
